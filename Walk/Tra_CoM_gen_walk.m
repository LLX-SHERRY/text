...
A=[ 1 Ts 1/2*Ts^2;  % 系统离散动态矩阵 A
    0 1  Ts;
    0 0  1];
B=[1/6*Ts^3;1/2*Ts^2;Ts]; % 输入矩阵 B
C=[1 0 -zc/g];  % 输出矩阵 C（ZMP 约束）  lipm离散状态方程xk+1 =A xk +B uk  𝑝𝑍𝑀𝑃=𝐶𝑑⋅𝑥𝑘
D=0;
Q=1;
R=1e-6;
NL=500;
...
z=z(1,1:end-1);
z=[z,zc*ones(1,Nk_total-Nk_ready)];%%zc一定的
%% x,y方向
if  0
    [X,~,~]=idare(A,B,Q,R,[],[]);%%%黎卡提
    K=(R+B'*X*B)^(-1)*B'*X*A;
    ...
    C=[1 0 0 0];
    [X,~,~]=idare(A,B,Q,R,[],[]);
    K=(R+B'*X*B)^(-1)*B'*X*A; %LQR 控制器的增益公式
    for ii=1:NL%NL预测时域长度
        f(1,ii)=(R+B'*X*B)^(-1)*B'*((A-B*K)')^(ii-1)*C'*Q;%f 是前馈增益，用于生成参考轨迹的预测控制。
    end
%     plot(f)
    x(:,1)=[px(1) 0 0 0]';
    y(:,1)=[py(1) 0 0 0]';
